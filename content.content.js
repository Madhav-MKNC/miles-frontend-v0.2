function fetchMessages() {
    ...
    const chats = Array.from(chatElements)
      ...
      .map(element => {
        ...
        const userLabelSpan = element.parentElement.parentElement.querySelector('span[aria-label]:first-of-type');
        const userLabel = userLabelSpan ? userLabelSpan.getAttribute('aria-label') : "";
  
        const messageElement = element.querySelector('.selectable-text');
        const messageText = messageElement ? messageElement.innerText : '';
  
        // If the userLabel is "You:", then set the thisUser
        if (userLabel.includes("You:")) {
          thisUser = username;
        }
